    ...
    >
      ...
      <div className="flex flex-col items-center px-3 pt-3">
        <div className="flex items-center justify-center">
          <img
            ...
            alt="WebSeeder Logo"
            className="rounded-full shadow-md w-10 h-10 md:w-14 md:h-14"
          />

          <motion.span
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="hidden md:inline-block ml-2 pt-2 font-semibold text-sm text-gray-800"
          >
            WebSeeder
          </motion.span>
        </div>
      ...
      <nav className="flex-1 flex flex-col text-sm space-y-1 mt-4 px-3 overflow-y-auto">
        {menuItems.map((item, index) => (
          <NavLink
            key={index}
            to={item.path}
            end={item.path === "/projects" ? false : true}
            className={({ isActive }) =>
      ...
      <div className="border-t border-gray-200 p-3 hidden md:block">
        ...
        >
          ...
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="ml-2 text-sm font-medium">
            Logout
          </motion.span>
        </div>
      </div>
      ...
      <div
        onClick={handleLogout}
        className="md:hidden flex justify-center py-4 border-t border-gray-200 cursor-pointer hover:bg-red-100"
      >
